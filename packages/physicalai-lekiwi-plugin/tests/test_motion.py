"""Tests for the motion/observation helpers in physicalai_lekiwi_plugin.motion."""
...
from physicalai_lekiwi_plugin.motion import HoldPoseSource, JointLogger, SineWaveSource
...
class TestSineWaveSource:
    def test_update_returns_sine_commands(self) -> None:
        ...
        with patch("physicalai_lekiwi_plugin.motion.time.monotonic", side_effect=[0.0, 0.5]):
    ...
    def test_joint_amplitudes_and_phase_offsets(self) -> None:
        ...
        with patch("physicalai_lekiwi_plugin.motion.time.monotonic", side_effect=[0.0, 1.0]):
            source.connect(bus=object(), session_id="s")
            action = source.update(obs, {}, 0)
    ...
    def test_export_config_roundtrip(self) -> None:
        from physicalai_lekiwi_plugin.motion import SineWaveSource

        cfg = to_config(SineWaveSource(frequency=0.5))
        assert cfg["class_path"] == "physicalai_lekiwi_plugin.motion.SineWaveSource"
...
class TestHoldPoseSource:
    ...
    def test_export_config_roundtrip(self) -> None:
        from physicalai_lekiwi_plugin.motion import HoldPoseSource

        cfg = to_config(HoldPoseSource())
        assert cfg["class_path"] == "physicalai_lekiwi_plugin.motion.HoldPoseSource"
...
class TestJointLogger:
    ...
    def test_export_config_roundtrip(self) -> None:
        from physicalai_lekiwi_plugin.motion import JointLogger

        cfg = to_config(JointLogger(throttle_steps=3))
        assert cfg["class_path"] == "physicalai_lekiwi_plugin.motion.JointLogger"
