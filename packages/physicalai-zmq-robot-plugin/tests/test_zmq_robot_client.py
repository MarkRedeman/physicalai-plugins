...
def mock_socket():
    ...
    socket.recv_string.return_value = json.dumps({"event": "features_read", "features": ["j1", "j2", "j3"]})
    return socket
...
class TestZMQRobotConnect:
    def test_connect_success(self, mock_context, mock_socket) -> None:
        with patch("physicalai_zmq_robot_plugin.zmq_robot.zmq.Context", return_value=mock_context):
    ...
    def test_connect_no_features(self, mock_context, mock_socket) -> None:
...
class TestZMQRobotLifecycle:
    ...
    def test_context_manager(self, mock_context, mock_socket) -> None:
...
class TestZMQRobotObservation:
    def test_get_observation(self, robot, mock_socket) -> None:
        mock_socket.recv_string.return_value = json.dumps({"state": {"j1": 10.0, "j2": 20.0, "j3": 30.0}})
        obs = robot.get_observation()
        assert np.allclose(obs.joint_positions, np.array([10.0, 20.0, 30.0]))

    def test_get_observation_timestamp(self, robot, mock_socket) -> None:
        mock_socket.recv_string.return_value = json.dumps({"state": {"j1": 0.0, "j2": 0.0, "j3": 0.0}})
        import time
    ...
    def test_get_observation_empty_state(self, robot, mock_socket) -> None:
...
class TestZMQRobotActions:
    def test_send_action(self, robot, mock_socket) -> None:
        mock_socket.recv_string.return_value = json.dumps({"event": "joints_state_was_set"})
    ...
    def test_enable_torque(self, robot, mock_socket) -> None:
        mock_socket.recv_string.return_value = json.dumps({"event": "torque_was_enabled"})
        robot.enable_torque()
        sent_str = mock_socket.send_string.call_args[0][0]
        sent = json.loads(sent_str)
        assert sent["command"] == "enable_torque"

    def test_disable_torque(self, robot, mock_socket) -> None:
        mock_socket.recv_string.return_value = json.dumps({"event": "torque_was_disabled"})
        robot.disable_torque()
        sent_str = mock_socket.send_string.call_args[0][0]
        sent = json.loads(sent_str)
        assert sent["command"] == "disable_torque"

    def test_send_action_shape_mismatch(self, robot, mock_socket) -> None:
        with pytest.raises((ValueError, IndexError)):
            robot.send_action(np.array([1.0, 2.0]))

    def test_zmq_error_raised(self, robot, mock_socket) -> None:
